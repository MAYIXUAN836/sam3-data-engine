...
def _do_matching(cost, repeats=1, return_tgt_indices=False, do_filtering=False):
    # Ensure we have a NumPy array and sanitize any non-finite entries
    # (NaN/Inf) that can cause `linear_sum_assignment` to fail.
    cost = np.asarray(cost)
    if not np.isfinite(cost).all():
        # Replace NaN/Inf with a very large finite value so that matching
        # still succeeds but these entries are effectively worst choices.
