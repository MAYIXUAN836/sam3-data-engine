...
class CheckpointConf:
    ...
    best_train_loss_key: str = "Losses/train_all_loss"
    best_train_ckpt_name: str = "best_train_loss"
...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        decoder_only_finetune: bool = False,
    ):
        ...
        self.decoder_only_finetune = decoder_only_finetune
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None):
        ...
        if checkpoint_names is None:
            # Keep both legacy resume target and explicit latest alias updated each epoch.
    ...
    def run_train(self):
        while self.epoch < self.max_epochs:
            ...
            self._maybe_save_best_train_loss_ckpt(outs)
            ...
            if self.distributed_rank == 0:
                ...
                self._update_loss_curve_plot()

            self.epoch += 1
        # epoch was incremented in the loop but the val step runs out of the loop
        self.epoch -= 1

    def _update_loss_curve_plot(self):
        train_stats = os.path.join(self.logging_conf.log_dir, "train_stats.json")
        if not os.path.isfile(train_stats):
            return
        ...
        with open(train_stats, "r") as f:
            for line in f:
                ...
                if epoch is None or loss is None:
                    continue
                train_epoch_to_loss[epoch] = loss

        if not train_epoch_to_loss:
            return
#加了一个每一个epoch都更新一下loss curve的函数，保存在训练日志目录下，文件名叫loss_curve.png, 这样就可以在训练过程中实时看到loss曲线了
        val_stats = os.path.join(self.logging_conf.log_dir, "val_stats.json")
        val_epoch_to_loss = {}
        if os.path.isfile(val_stats):
            with open(val_stats, "r") as f:
                for line in f:
                    line = line.strip()
                    if not line:
                        continue
                    try:
                        ...
                        continue
                    epoch = record.get("Trainer/epoch")
                    loss = record.get("Losses/val_all_loss")
                    if epoch is None or loss is None:
                        continue
                    val_epoch_to_loss[epoch] = loss

        epochs = sorted(train_epoch_to_loss.keys())
        train_losses = [train_epoch_to_loss[e] for e in epochs]

        plt.figure(figsize=(7, 4.5))
        plt.plot(epochs, train_losses, marker="o", label="train_all_loss")
        if val_epoch_to_loss:
            val_epochs = sorted(val_epoch_to_loss.keys())
            val_losses = [val_epoch_to_loss[e] for e in val_epochs]
            plt.plot(val_epochs, val_losses, marker="s", label="val_all_loss")
        ...
        exp_dir = os.path.dirname(self.logging_conf.log_dir)
        out_path = os.path.join(exp_dir, "loss_curve.png")
    ...
    def _maybe_save_best_train_loss_ckpt(self, outs: Dict[str, Any]):
        if not self.checkpoint_conf.save_best_train_loss:
            return

        metric_key = self.checkpoint_conf.best_train_loss_key
        if metric_key not in outs:
            return

        metric_val = outs[metric_key]
        ...
        try:
            ...
            logging.warning(
                "[Checkpoint] Skip saving best train ckpt: non-float metric key=%s val=%s",
                metric_key,
                str(metric_val),
            )
            return
        ...
        if not math.isfinite(metric_val):
            logging.warning(
                "[Checkpoint] Skip saving best train ckpt: non-finite metric key=%s val=%s",
                metric_key,
                str(metric_val),
            )
            return

        best_key = f"best_train_loss/{metric_key}"
        best_epoch_key = f"best_train_loss/{metric_key}_epoch"
        prev_best = float(self.best_meter_values.get(best_key, float("inf")))

        if metric_val < prev_best:
            self.best_meter_values[best_key] = metric_val
            self.best_meter_values[best_epoch_key] = int(self.epoch + 1)
            ckpt_name = self.checkpoint_conf.best_train_ckpt_name
            self.save_checkpoint(self.epoch + 1, checkpoint_names=[ckpt_name])
            logging.info(
                "[Checkpoint] Saved new best train ckpt: %s (epoch=%d, %s=%.6f)",
                ckpt_name,
                int(self.epoch + 1),
                metric_key,
                metric_val,
    ...
    def _configure_decoder_only_finetune(self):
        # Freeze everything first, then only unfreeze decoder-related weights.
        ...
        for name, p in self.model.named_parameters():
            ...
            is_decoder_related = (
                "decoder" in name_l
                or "query_feat" in name_l
                or "query_pos" in name_l
                or "bbox_embed" in name_l
                or "class_embed" in name_l
            )
            is_backbone = (
                "vision_backbone" in name_l or "language_backbone" in name_l
            )
            if is_decoder_related and not is_backbone:
                p.requires_grad_(True)
                trainable_param_names.append(name)

        if len(trainable_param_names) == 0:
            raise RuntimeError(
                "decoder_only_finetune=True 但未匹配到可训练参数，请检查关键字配置。"
            )

        total = sum(p.numel() for p in self.model.parameters())
        trainable = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        logging.info(
            "[Fine-tune] Decoder-only: trainable %.2fM / %.2fM params (%.2f%%)",
            trainable / 1e6,
            total / 1e6,
    ...
    def _setup_components(self):
        ...
        self.model = instantiate(self.model_conf, _convert_="all")
        if self.enable_lora:
            if self.decoder_only_finetune:
                logging.warning(
                    "[Fine-tune] enable_lora=True，将忽略 decoder_only_finetune。"
                )
            import sys as _sys

            _sys.path.insert(0, "/home/projectx/Sam3_data_engine/scripts")
            from lora_injection import apply_lora_to_sam3

            logging.info("[LoRA] 注入中...")
            apply_lora_to_sam3(self.model, r=self.lora_r, alpha=self.lora_alpha)
            logging.info("[LoRA] 完成！")
        elif self.decoder_only_finetune:
            logging.info("[Fine-tune] LoRA 已禁用，启用 decoder-only 训练。")
            self._configure_decoder_only_finetune()
        else:
            logging.info("[Fine-tune] LoRA 已禁用，采用全参数训练。")
        print_model_summary(self.model)
