...
git remote add origin git@github.com:YOUR_NAME/sam3-data-engine.git
...
huggingface-cli upload YOUR_NAME/sam3-data-engine-checkpoints \
...
访问 `https://huggingface.co/YOUR_NAME/sam3-data-engine-checkpoints`
...
api = HfApi()
repo_id = "YOUR_NAME/sam3-data-engine-checkpoints"
...
**Q: 怎样禁用自动权重下载（走离线模式）？**  
A: 用户运行 `DOWNLOAD_WEIGHTS=0 bash setup.sh` 即可跳过权重下载。
