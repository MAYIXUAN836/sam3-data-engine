...
def apply_brightness_contrast(rgb, delta_b, delta_c):
    ...
    Contrast   ±10%: per-channel 均值做基准，避免全图均值偏差
    只作用于 RGB 图，mask 不变
    """
    ...
    if delta_c != 0.0:
        # FIX: 改为 per-channel 均值，避免卫星图不同区域亮度差异大导致的偏差
        mean = img.mean(axis=(0, 1), keepdims=True)  # shape: (1, 1, 3)
...
def apply_grid_distortion(rgb, mask, num_steps=3, distort_limit=0.05):
    """
    真正的局部非线性几何扭曲，模拟不同传感器的几何畸变。
    FIX: 原实现用 findHomography+RANSAC 是全局线性变换，局部扰动信息丢失。
         改为逐像素 remap，通过插值控制点偏移实现真正的局部形变。
    """
    h, w = rgb.shape[:2]

    # 生成 grid 控制点坐标
    xs = np.linspace(0, w, num_steps + 1, dtype=np.float32)
    ys = np.linspace(0, h, num_steps + 1, dtype=np.float32)
    ...
    interp_x = RegularGridInterpolator(
        (ys, xs), dx, method='linear', bounds_error=False, fill_value=0.0)
    interp_y = RegularGridInterpolator(
        (ys, xs), dy, method='linear', bounds_error=False, fill_value=0.0)

    yy, xx = np.mgrid[0:h, 0:w].astype(np.float32)
    pts = np.stack([yy.ravel(), xx.ravel()], axis=1)

    map_x = (xx + interp_x(pts).reshape(h, w)).clip(0, w - 1).astype(np.float32)
    map_y = (yy + interp_y(pts).reshape(h, w)).clip(0, h - 1).astype(np.float32)

    rgb_out = cv2.remap(rgb, map_x, map_y,
...
def make_mosaic(pairs, out_rgb_dir, out_mask_dir, crop_size, num_mosaic):
    """
    Mosaic 4图拼接。
    FIX: 原实现先 resize 到 half×half 再拼，损失了 75% 分辨率。
         改为从每张图随机 crop 512×512 patch 再拼，无分辨率损失。
    """
    n    = len(pairs)
    half = crop_size // 2
    count = 0
    ...
    for i in tqdm(range(num_mosaic), desc="Mosaic"):
        ...
        for idx in indices:
            ...
            # FIX: 随机 crop half×half patch，不做 resize
            h, w = rgb.shape[:2]
            if h < half or w < half:
                # 极少数情况图片小于 half，fallback resize
                ...
                y = random.randint(0, h - half)
                x = random.randint(0, w - half)
                rgb  = rgb[y:y+half,  x:x+half]
                mask = mask[y:y+half, x:x+half]
